...
use crate::model::{AnomalyRow, ReportSummary};
...
pub async fn generate_daily_report(state: &AppState) -> Result<()> {
    ...
    let summary = state.repo.fetch_summary(&date).await?;
    let detail = state.repo.fetch_anomalies(&date, None).await?;
...
fn next_report_time(config: &crate::config::AppConfig) -> DateTime<Local> {
    let now = Local::now();
