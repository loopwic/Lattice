...
use crate::model::AnomalyRow;

pub fn spawn_alerts(config: &AppConfig, anomalies: &[AnomalyRow]) {
...
pub async fn check_alert_target(config: &AppConfig) -> Result<()> {
...
async fn send_alerts(config: &AppConfig, alerts: &[AnomalyRow]) -> Result<()> {
    let url = resolve_alert_url(config)?;
    if url.starts_with("ws://") || url.starts_with("wss://") {
...
async fn send_http_alerts(config: &AppConfig, url: &str, alerts: &[AnomalyRow]) -> Result<()> {
    let template = config.alert_webhook_template.as_deref().unwrap_or(
        r#"{"message":"稀有物资告警 {total} 条\n{lines}"}"#,
    );
...
async fn check_http_target(config: &AppConfig, url: &str) -> Result<()> {
    let client = Client::builder()
        .timeout(Duration::from_secs(config.request_timeout_seconds.max(3)))
        .build()?;
...
async fn check_ws_target(config: &AppConfig, url: &str) -> Result<()> {
...
async fn send_ws_alerts(config: &AppConfig, url: &str, alerts: &[AnomalyRow]) -> Result<()> {
    let group_id = config.alert_group_id.ok_or_else(|| anyhow::anyhow!("alert_group_id not configured"))?;
...
async fn try_ws_check(url: &str, token: Option<&str>, use_query: bool) -> Result<()> {
    let mut request = if use_query {
        add_access_token_query(url, token)
            .into_client_request()?
    } else {
...
fn resolve_alert_url(config: &AppConfig) -> Result<String> {
